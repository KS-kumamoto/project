...
フロントエンドにReact（Vite）、バックエンドにSpring Bootを利用したフルスタックのWebアプリケーションです。  
　※バックエンドのデプロイがRenderフリープランのため、起動に1分程度かかる場合があります。  

👉 **[実際のアプリをプレビューする（ここをクリック）](https://project-ivory-rho-74.vercel.app/)**

## 📖 プロジェクトの概要

職業訓練で作成したX(旧twitter)の再現アプリと同等の機能をフレームワークを使用してセキュアに再構築しました。  
現場で使われる技術について理解し、GitHubやデプロイ後の挙動についての知見を得る事を目的としています。  
Google AntigravityによるAI駆動開発を行い、2日で自動テスト+手動での最終動作確認まで完了しています。  
  　※仕様書はproject内に保管、処理フローはアプリ起動後にロゴの右から📄 Data Flow Docsを参照ください
...
- **別途レポートを作成**:  
  フレームワーク依存の課題等も含め、セキュリティ対策について別途レポートを作成しました。  
  Security Report.mdを参照ください。


## 🌈今後の展望

- **フレームワークの完全理解**:
  処理フローを追って一通りの動作を理解した段階で、細部の仕様や構文についての理解が不完全なままです。
  機能追加や改修を通して更に理解を深めていきたいと思います。

- **Databaseのデータ永続化とユーザーごとの権限を追加し、個人用のナレッジ記録アプリへ改修**:
  基本構造を変えないまま転用可能だと考えています。
  なるべく料金がかからない方法で完成させるつもりです。  
